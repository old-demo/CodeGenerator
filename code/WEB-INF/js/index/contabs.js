...
$(function () {
	...
	$('.J_tabRefresh').on('click', tabRefresh);
    $('.J_menuTabs').on('dblclick', '.J_menuTab', tabRefresh);
    
    //刷新iframe
    function tabRefresh(){
		var id = $('.J_menuTab.active').attr('data-id');
    ...
    function tabLocation(){
		var data_id = $('.J_menuTab.active').attr('data-id');
	    $(".J_menuItem").each(function () {
        	if(data_id == $(this).attr('href')) {
				if($(this).parent().parent().is("#side-menu")){
					$(this).focus();
				} else {
					var parent_2 = $(this).parent().parent();
					parent_2.parent().addClass("active");
					parent_2.addClass("in").attr("aria-expanded", "true");
					$(this).focus();
				}
        	} 
	    });
